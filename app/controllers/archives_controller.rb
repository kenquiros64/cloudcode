class ArchivesController < ApplicationController
  ...
  # GET /archives/1.json
  def show
    if user_signed_in? and current_user.archives.find_by_id(params[:id]) 
    ...
    if archive 
      respond_to do |format|
        if @archive.update(archive_params)
          format.html { redirect_to @archive, notice: 'Archive was successfully save.' }
  ...
  def destroy
    @archive = current_user.archives.find(params[:id]) 
    @parent_folder = @archive.folder #grabbing the parent folder before deleting the record
    @archive.destroy
